...
def generate_user_threat_model_pdf(workshop_config, user_answers, total_score, max_score):
    """Generate PDF of user's threat model (what they submitted)"""
    try:
        buffer = BytesIO()
        ...
        # Cover Page
        story.append(Paragraph(f"STRIDE Threat Model Report", title_style))
        story.append(Spacer(1, 0.2*inch))
        story.append(Paragraph(f"{workshop_config['name']}", styles['Heading2']))
        story.append(Paragraph(f"{workshop_config['scenario']['title']}", styles['Heading3']))
        ...
        metadata = [
            ...
            ['Score:', f"{total_score}/{max_score} ({(total_score/max_score*100):.1f}%)"]
        ]
        ...
        final_score_pct = (total_score / max_score) * 100 if max_score > 0 else 0
        
        summary_text = f"""
        This threat model was created for {workshop_config['scenario']['title']}, 
        a {workshop_config.get('architecture_type', 'system')}. The analysis identified 
        {len(user_answers)} threats using the STRIDE methodology. The overall assessment 
        achieved a score of {final_score_pct:.1f}%, indicating {
            'excellent' if final_score_pct >= 90 else
            'good' if final_score_pct >= 75 else
            'satisfactory' if final_score_pct >= 60 else
            'needs improvement'
        } understanding of threat modeling principles.
        """
        ...
        story.append(Paragraph(f"<b>Description:</b> {workshop_config['scenario']['description']}", styles['Normal']))
        story.append(Paragraph(f"<b>Business Context:</b> {workshop_config['scenario']['business_context']}", styles['Normal']))
        story.append(Spacer(1, 0.1*inch))
        
        # Assets
        story.append(Paragraph("<b>Critical Assets:</b>", styles['Normal']))
        for asset in workshop_config['scenario']['assets']:
            story.append(Paragraph(f"• {asset}", styles['Normal']))
        story.append(Spacer(1, 0.1*inch))
        
        # Compliance
        story.append(Paragraph("<b>Compliance Requirements:</b>", styles['Normal']))
        for comp in workshop_config['scenario']['compliance']:
            story.append(Paragraph(f"• {comp}", styles['Normal']))
        story.append(PageBreak())
        
        # Identified Threats
        story.append(Paragraph("2. Identified Threats", heading_style))
        ...
        for idx, answer in enumerate(user_answers, 1):
            ...
            # Threat header
            threat_title = f"Threat {idx}: {answer.get('matched_threat_id', 'N/A')}"
            story.append(Paragraph(threat_title, styles['Heading3']))
            
            # Threat details table
            threat_data = [
                ['Component:', answer['component']],
                ['STRIDE Category:', answer['stride']],
                ['Likelihood:', answer['likelihood']],
                ['Impact:', answer['impact']],
                ['Assessment Score:', f"{answer['score']}/{answer['max_score']} ({score_pct:.0f}%)"]
            ]
            ...
            if answer.get('selected_mitigations'):
                ...
                for mit in answer['selected_mitigations']:
                    story.append(Paragraph(f"• {mit}", styles['Normal']))
                story.append(Spacer(1, 0.2*inch))
        ...
        correct_count = sum(1 for a in user_answers if (a["score"] / a["max_score"]) >= 0.8)
        partial_count = sum(1 for a in user_answers if 0.5 <= (a["score"] / a["max_score"]) < 0.8)
        incorrect_count = sum(1 for a in user_answers if (a["score"] / a["max_score"]) < 0.5)
        
        perf_data = [
            ['Category', 'Count', 'Percentage'],
            ['Excellent (80%+)', str(correct_count), f"{(correct_count/len(user_answers)*100):.1f}%"],
            ['Partial (50-79%)', str(partial_count), f"{(partial_count/len(user_answers)*100):.1f}%"],
            ['Needs Review (<50%)', str(incorrect_count), f"{(incorrect_count/len(user_answers)*100):.1f}%"]
        ]
        ...
        st.error(f"Error generating user PDF: {str(e)}")
        return None

def generate_complete_threat_model_pdf(workshop_config, workshop_id):
    """Generate complete/perfect threat model PDF for reference"""
    try:
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=letter, topMargin=0.75*inch, bottomMargin=0.75*inch)
        styles = getSampleStyleSheet()
        ...
        # Get all predefined threats for this workshop
        all_threats = PREDEFINED_THREATS.get(workshop_id, [])
        attack_tree = ATTACK_TREES.get(workshop_id, {})
        ...
        # Cover metadata
        cover_data = [
            ['Report Type:', 'Complete Reference Model'],
            ['Workshop Level:', workshop_config['level']],
            ['Architecture:', workshop_config.get('architecture_type', 'N/A')],
            ['Prepared By:', 'STRIDE Learning Lab'],
        ...
        exec_summary = f"""
        This comprehensive threat model provides a detailed security analysis of {workshop_config['scenario']['title']}, 
        a {workshop_config.get('architecture_type', 'system')}. The analysis employs industry-standard methodologies 
        including STRIDE threat classification and Attack Tree modeling to systematically identify and assess security risks.
        <br/><br/>
        <b>Key Findings:</b><br/>
        • Identified {len(all_threats)} distinct threat scenarios across all STRIDE categories<br/>
        • Analyzed {len(workshop_config['scenario']['trust_boundaries'])} critical trust boundaries<br/>
        • Assessed {len(workshop_config['scenario']['components'])} system components<br/>
        • Documented comprehensive mitigation strategies for each identified threat<br/>
        <br/>
        <b>Risk Profile:</b><br/>
        The system faces risks spanning all six STRIDE categories. Critical priorities include:
        """
        ...
        for category, count in sorted(stride_counts.items(), key=lambda x: x[1], reverse=True)[:3]:
            story.append(Paragraph(f"• {category}: {count} identified threats", styles['Normal']))
        
        story.append(PageBreak())
        
        # 2. SYSTEM OVERVIEW
        story.append(Paragraph("2. System Overview", heading_style))
        
        story.append(Paragraph("2.1 Business Context", subheading_style))
        story.append(Paragraph(workshop_config['scenario']['description'], styles['Normal']))
        story.append(Paragraph(workshop_config['scenario']['business_context'], styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        story.append(Paragraph("2.2 Security Objectives", subheading_style))
        for obj in workshop_config['scenario']['objectives']:
            story.append(Paragraph(f"• {obj}", styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        story.append(Paragraph("2.3 Critical Assets", subheading_style))
        assets_text = "The following assets require protection:"
        story.append(Paragraph(assets_text, styles['Normal']))
        for asset in workshop_config['scenario']['assets']:
            story.append(Paragraph(f"• {asset}", styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        story.append(Paragraph("2.4 Compliance Requirements", subheading_style))
        story.append(Paragraph("This system must comply with:", styles['Normal']))
        for comp in workshop_config['scenario']['compliance']:
            story.append(Paragraph(f"• {comp}", styles['Normal']))
        ...
        # 3. ARCHITECTURE DECOMPOSITION
        story.append(Paragraph("3. Architecture Decomposition", heading_style))
        
        story.append(Paragraph("3.1 System Components", subheading_style))
        
        # Components table
        comp_data = [['Component', 'Type', 'Description']]
        for comp in workshop_config['scenario']['components']:
            comp_type = comp['type'].replace('_', ' ').title()
            comp_data.append([comp['name'], comp_type, comp['description']])
        
        comp_table = Table(comp_data, colWidths=[1.5*inch, 1.5*inch, 3.5*inch])
        ...
        # Data flows table
        flow_data = [['Source', 'Destination', 'Data', 'Protocol']]
        for flow in workshop_config['scenario']['data_flows']:
            flow_data.append([flow['source'], flow['destination'], flow['data'], flow['protocol']])
        
        flow_table = Table(flow_data, colWidths=[1.5*inch, 1.5*inch, 2*inch, 1.5*inch])
        ...
        story.append(Paragraph("3.3 Trust Boundaries", subheading_style))
        for boundary in workshop_config['scenario']['trust_boundaries']:
            story.append(Paragraph(f"<b>{boundary['name']}:</b> {boundary['description']}", styles['Normal']))
        
        story.append(PageBreak())
        
        # 4. ATTACK TREE ANALYSIS
        if attack_tree:
            story.append(Paragraph("4. Attack Tree Analysis", heading_style))
            story.append(Paragraph(attack_tree.get('description', ''), styles['Normal']))
            story.append(Spacer(1, 0.2*inch))
            story.append(Paragraph("Attack paths identified through systematic analysis demonstrate multiple vectors an adversary could exploit. Each path has been evaluated for difficulty and likelihood.", styles['Normal']))
        
        story.append(PageBreak())
        
        # 5. STRIDE THREAT ASSESSMENT
        story.append(Paragraph("5. STRIDE Threat Assessment", heading_style))
        
        stride_summary = """
        The STRIDE methodology provides systematic coverage across six threat categories:
        <br/>• <b>Spoofing:</b> Identity impersonation and authentication bypass
        <br/>• <b>Tampering:</b> Unauthorized data modification
        <br/>• <b>Repudiation:</b> Denying actions without proof
        <br/>• <b>Information Disclosure:</b> Unauthorized data access
        <br/>• <b>Denial of Service:</b> Availability disruption
        <br/>• <b>Elevation of Privilege:</b> Unauthorized access escalation
        """
        story.append(Paragraph(stride_summary, styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        # STRIDE distribution table
        stride_dist = [['STRIDE Category', 'Threat Count', 'Percentage']]
        total_threats = len(all_threats)
        for category in ['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege']:
            count = sum(1 for t in all_threats if t['stride'] == category)
            pct = (count / total_threats * 100) if total_threats > 0 else 0
            stride_dist.append([category, str(count), f"{pct:.1f}%"])
        
        stride_table = Table(stride_dist, colWidths=[3*inch, 1.5*inch, 1.5*inch])
        ...
        # 6. DETAILED THREAT CATALOG
        story.append(Paragraph("6. Detailed Threat Catalog", heading_style))
        
        for idx, threat in enumerate(all_threats, 1):
            story.append(Paragraph(f"6.{idx} {threat['id']}: {threat.get('threat', 'Threat')}", subheading_style))
            ...
            summary_table = Table(threat_summary, colWidths=[2*inch, 4*inch])
            ...
            story.append(Paragraph(threat.get('explanation', 'N/A'), styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
            
            # Risk Rationale
            story.append(Paragraph("<b>Risk Rationale:</b>", styles['Normal']))
            story.append(Paragraph(threat.get('why_this_risk', 'N/A'), styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
            
            # Mitigation Strategies
            story.append(Paragraph("<b>Recommended Mitigations:</b>", styles['Normal']))
            for mit in threat.get('correct_mitigations', []):
                story.append(Paragraph(f"• {mit}", styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
            
            # Why Controls Work
            story.append(Paragraph("<b>Control Effectiveness:</b>", styles['Normal']))
            story.append(Paragraph(threat.get('why_these_controls', 'N/A'), styles['Normal']))
            story.append(Spacer(1, 0.1*inch))
            
            # Real-world Example
            if threat.get('real_world'):
                story.append(Paragraph("<b>Real-World Example:</b>", styles['Normal']))
                story.append(Paragraph(threat.get('real_world'), styles['Normal']))
                story.append(Spacer(1, 0.1*inch))
            
            # Compliance
            if threat.get('compliance'):
                story.append(Paragraph(f"<b>Compliance References:</b> {threat.get('compliance')}", styles['Normal']))
            
            story.append(Spacer(1, 0.2*inch))
            
            # Page break every 2 threats to keep readable
        ...
        # 7. MITIGATION STRATEGIES
        story.append(Paragraph("7. Consolidated Mitigation Strategies", heading_style))
        
        # Collect all unique mitigations
        all_mitigations = set()
        for threat in all_threats:
            all_mitigations.update(threat.get('correct_mitigations', []))
        
        story.append(Paragraph(f"A total of {len(all_mitigations)} distinct security controls have been identified across all threats. Implementation priority should be based on threat criticality and coverage.", styles['Normal']))
        story.append(Spacer(1, 0.2*inch))
        
        # Group mitigations by category
        categories = {
            'Authentication & Identity': ['MFA', 'authentication', 'password', 'OAuth', 'SAML', 'identity', 'certificate', 'mTLS'],
            'Input Validation & Sanitization': ['validation', 'sanitization', 'parameterized', 'XSS', 'injection', 'encoding'],
            'Encryption & Cryptography': ['encryption', 'TLS', 'AES', 'KMS', 'crypto', 'signing', 'HMAC'],
            'Access Control': ['authorization', 'RBAC', 'ABAC', 'least privilege', 'access control', 'RLS'],
            'Logging & Monitoring': ['logging', 'audit', 'SIEM', 'monitoring', 'tracing', 'correlation'],
            'Resilience & Availability': ['rate limiting', 'scaling', 'WAF', 'DDoS', 'circuit breaker', 'timeout'],
        }
        
        for cat_name, keywords in categories.items():
            cat_mitigations = [m for m in all_mitigations if any(kw.lower() in m.lower() for kw in keywords)]
            if cat_mitigations:
                story.append(Paragraph(f"<b>{cat_name}:</b>", styles['Normal']))
                for mit in sorted(cat_mitigations)[:5]:  # Top 5 per category
                    story.append(Paragraph(f"• {mit}", styles['Normal']))
                story.append(Spacer(1, 0.1*inch))
        ...
        # 8. RISK SUMMARY
        story.append(Paragraph("8. Risk Summary Matrix", heading_style))
        
        # Risk distribution
        risk_combos = {}
        for threat in all_threats:
            key = f"{threat['likelihood']}/{threat['impact']}"
            risk_combos[key] = risk_combos.get(key, 0) + 1
        
        risk_data = [['Likelihood', 'Impact', 'Count', 'Priority']]
        priority_map = {
            'High/Critical': 'P0 - Critical',
            'High/High': 'P0 - Critical',
            'Medium/Critical': 'P0 - Critical',
            'Medium/High': 'P1 - High',
            'High/Medium': 'P1 - High',
            'Medium/Medium': 'P2 - Medium',
            'Low/Critical': 'P1 - High',
            'Low/High': 'P2 - Medium',
            'Low/Medium': 'P3 - Low',
        }
        
        for combo, count in sorted(risk_combos.items(), key=lambda x: x[1], reverse=True):
            likelihood, impact = combo.split('/')
            priority = priority_map.get(combo, 'P3 - Low')
            risk_data.append([likelihood, impact, str(count), priority])
        
        risk_table = Table(risk_data, colWidths=[1.5*inch, 1.5*inch, 1*inch, 2*inch])
        ...
        # 9. COMPLIANCE MAPPING
        story.append(Paragraph("9. Compliance Mapping", heading_style))
        
        compliance_text = f"""
        This threat model supports compliance with {', '.join(workshop_config['scenario']['compliance'])}. 
        Each identified threat includes relevant compliance framework references demonstrating how security 
        controls map to regulatory requirements.
        """
        ...
        # 10. RECOMMENDATIONS
        story.append(Paragraph("10. Recommendations & Next Steps", heading_style))
        
        recommendations = [
            "Implement all P0 (Critical) mitigations immediately",
            "Establish continuous threat modeling process with quarterly reviews",
            "Conduct penetration testing to validate control effectiveness",
            "Integrate threat model findings into sprint planning and backlog",
            "Provide security training to development team on identified threat patterns",
            "Establish metrics to track mitigation implementation progress",
            "Review and update threat model when architecture changes occur"
        ]
        ...
        conclusion = f"""
        This comprehensive threat model provides a solid foundation for secure {workshop_config.get('architecture_type', 'system')} 
        development. By systematically addressing identified threats through the recommended mitigations, 
        the organization can significantly reduce security risk while maintaining compliance with applicable regulations.
        """
...
# STEP 5: ASSESS WITH THREAT-MAPPED DIAGRAM AND EXPLANATIONS (was Step 4)
    ...
    st.markdown("""
    <div class="info-box">
    <strong>Two Export Options Available:</strong><br>
    • <strong>Your Threat Model:</strong> PDF of your submitted analysis and scores<br>
    • <strong>Complete Reference Model:</strong> Enterprise-grade threat model with all threats, mitigations, and compliance mappings
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        # Generate user's threat model PDF
        with st.spinner("Generating your threat model PDF..."):
        ...
        if user_pdf:
            st.download_button(
                "📄 Your Threat Model PDF",
                user_pdf,
                f"my_threat_model_ws{st.session_state.selected_workshop}_{datetime.now().strftime('%Y%m%d')}.pdf",
                "application/pdf",
                use_container_width=True,
                help="Your submitted analysis with scores"
            )
        else:
            st.error("Error generating user PDF")
    
    with col3:
        # Generate complete reference PDF
        with st.spinner("Generating complete reference PDF..."):
        ...
        if complete_pdf:
            st.download_button(
                ...
                complete_pdf,
                f"complete_threat_model_ws{st.session_state.selected_workshop}_{datetime.now().strftime('%Y%m%d')}.pdf",
                "application/pdf",
                use_container_width=True,
                help="Enterprise-grade complete threat model"
            )
        else:
            st.error("Error generating complete PDF")
