...
def render_architecture_svg(workshop_config, highlighted_threats=None, mode="architecture"):
    """
    Render a draw.io-style SVG architecture diagram dynamically from workshop data.

    mode: "architecture"  – clean layout showing components + flows + zones
          "threat"        – same but threat-affected nodes/edges highlighted red
          "stride"        – edges annotated with STRIDE rule labels
    """
    highlighted_threats = highlighted_threats or []

    scenario     = workshop_config["scenario"]
    components   = scenario["components"]
    data_flows   = scenario["data_flows"]
    ...
    # ── Layout: columns by zone score ──────────────────────────────────────
    zone_order_score = {
        "Not in Control of System": 0,
        "Minimal Trust":            1,
    ...
    for c in components:
        col_key = zone_order_score.get(c.get("zone", "Standard Application"), 2)
        cols[col_key].append(c)
    ...
    num_cols = max(cols.keys()) + 1 if cols else 1
    max_rows = max(len(v) for v in cols.values()) if cols else 1

    W = PAD_X * 2 + num_cols * COL_W
    H = PAD_Y * 2 + max_rows * ROW_H + 80  # +80 for legend

    # Compute node centres
    node_pos = {}
    for col_idx, comp_list in cols.items():
        for row_idx, comp in enumerate(comp_list):
            cx = PAD_X + col_idx * COL_W + COL_W // 2
            cy = PAD_Y + row_idx * ROW_H + NODE_H // 2 + 40  # +40 for zone banner
            node_pos[comp["name"]] = (cx, cy)

    # Map component name → meta
    comp_meta = {c["name"]: c for c in components}

    zone_score_map = {c["name"]: c.get("zone_score", 3) for c in components}
...
<defs>
  ...
  <marker id="arr_red" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
    <path d="M0,0 L0,6 L8,3 z" fill="#C62828"/>
  </marker>
  <marker id="arr_blue" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
...
''')
    ...
    # ── Zone column bands (background) ─────────────────────────────────────
    zone_band_drawn = set()
    for col_idx, comp_list in sorted(cols.items()):
        if not comp_list:
            continue
        zone_name = comp_list[0].get("zone", "Standard Application")
        if zone_name in zone_band_drawn:
            continue
        zone_band_drawn.add(zone_name)
        bx = PAD_X + col_idx * COL_W
        fill   = _zone_hex(zone_name)
        stroke = _zone_stroke(zone_name)
        score  = comp_list[0].get("zone_score", "?")
        band_h = H - PAD_Y - 80
        lines.append(
            f'<rect x="{bx}" y="{PAD_Y-10}" width="{COL_W}" height="{band_h}" rx="6" fill="{fill}" stroke="{stroke}" stroke-width="1.5" stroke-dasharray="6,3" opacity="0.5"/>')
        # Zone label at top of column
        lines.append(
            f'<text x="{bx + COL_W//2}" y="{PAD_Y+8}" text-anchor="middle" font-size="10" font-weight="bold" fill="{stroke}">{zone_name} (z{score})</text>')

    # Trust boundary vertical lines between columns
    used_cols = sorted(cols.keys())
    for i in range(len(used_cols) - 1):
        bx = PAD_X + (used_cols[i] + 1) * COL_W
        lines.append(
            f'<line x1="{bx}" y1="{PAD_Y-10}" x2="{bx}" y2="{H-90}" stroke="#7B1FA2" stroke-width="2" stroke-dasharray="8,4" opacity="0.7"/>')
        lines.append(
            f'<text x="{bx}" y="{PAD_Y+22}" text-anchor="middle" font-size="9" fill="#7B1FA2" transform="rotate(-90,{bx},{PAD_Y+22})">Trust Boundary</text>')
    ...
    for flow in data_flows:
        ...
        src_z  = zone_score_map.get(src, 3)
        dst_z  = zone_score_map.get(dst, 3)

        # STRIDE annotation for stride mode
        stride_ann = ""
        if mode == "stride":
            if src_z < dst_z:
                stride_ann = " [T]"
        ...
        color     = "#C62828" if is_thr else ("#1565C0" if mode == "stride" and stride_ann else "#555")
        marker_id = "arr_red" if is_thr else ("arr_blue" if mode == "stride" and stride_ann else "arr")
        sw        = "2.5" if is_thr else "1.5"

        # Slightly curve same-column flows
        if abs(x1 - x2) < 20:
            mx = x1 + 60
            lines.append(
                f'<path d="M{x1},{y1} Q{mx},{(y1+y2)//2} {x2},{y2}" fill="none" stroke="{color}" stroke-width="{sw}" marker-end="url(#{marker_id})"/>')
        else:
            lines.append(
                f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="{color}" stroke-width="{sw}" marker-end="url(#{marker_id})"/>')

        # Edge label
        lx = (x1 + x2) // 2
        ly = (y1 + y2) // 2 - 8
        label_text = f"{flow['data']}{stride_ann}"
        lines.append(
            f'<rect x="{lx-len(label_text)*3}" y="{ly-10}" width="{len(label_text)*6+4}" height="14" rx="3" fill="white" opacity="0.85"/>')
        lines.append(
            f'<text x="{lx}" y="{ly}" text-anchor="middle" font-size="9" fill="{color}">{label_text}</text>')
    ...
    for comp in components:
        ...
        fill    = "#FF8A80" if is_thr else _zone_hex(zone)
        stroke  = "#C62828" if is_thr else _zone_stroke(zone)
        sw      = "3" if is_thr else "2"
        ...
        if ctype == "external_entity":
            # Ellipse
            lines.append(
                f'<ellipse cx="{cx}" cy="{cy}" rx="{NODE_W//2}" ry="{NODE_H//2}" fill="{fill}" stroke="{stroke}" stroke-width="{sw}" filter="url(#shadow)"/>')
        elif ctype == "datastore":
            # Cylinder (rect + two ellipses)
            cap = 10
            lines.append(
                f'<rect x="{x0}" y="{y0+cap}" width="{NODE_W}" height="{NODE_H-cap*2}" fill="{fill}" stroke="{stroke}" stroke-width="{sw}" filter="url(#shadow)"/>')
            lines.append(
                f'<ellipse cx="{cx}" cy="{y0+cap}" rx="{NODE_W//2}" ry="{cap}" fill="{fill}" stroke="{stroke}" stroke-width="{sw}"/>')
            lines.append(
                f'<ellipse cx="{cx}" cy="{y0+NODE_H-cap}" rx="{NODE_W//2}" ry="{cap}" fill="{fill}" stroke="{stroke}" stroke-width="{sw}" opacity="0.6"/>')
        else:
            # Process – rounded rect
            lines.append(
                f'<rect x="{x0}" y="{y0}" width="{NODE_W}" height="{NODE_H}" rx="8" fill="{fill}" stroke="{stroke}" stroke-width="{sw}" filter="url(#shadow)"/>')

        # Threat badge
        if is_thr:
            lines.append(
                f'<circle cx="{x0+NODE_W}" cy="{y0}" r="10" fill="#C62828"/>')
            lines.append(
                f'<text x="{x0+NODE_W}" y="{y0+4}" text-anchor="middle" font-size="10" fill="white" font-weight="bold">!</text>')

        # Component name
        lines.append(
            f'<text x="{cx}" y="{cy-8}" text-anchor="middle" font-size="11" font-weight="bold" fill="#212121">{name}</text>')
        # Description
        lines.append(
            f'<text x="{cx}" y="{cy+8}" text-anchor="middle" font-size="9" fill="#555">{desc}</text>')
        # Zone score badge
        lines.append(
            f'<rect x="{x0+2}" y="{y0+NODE_H-16}" width="30" height="13" rx="3" fill="{stroke}" opacity="0.9"/>')
        lines.append(
            f'<text x="{x0+17}" y="{y0+NODE_H-5}" text-anchor="middle" font-size="8" fill="white" font-weight="bold">z{score}</text>')

    # ── Legend ─────────────────────────────────────────────────────────────
    legend_y = H - 72
    lines.append(f'<text x="{PAD_X}" y="{legend_y}" font-size="10" font-weight="bold" fill="#333">Legend:</text>')
    items = [
        ("⬭ External Entity", "#F5F5F5", "#9E9E9E"),
        ("▭ Process",          "#FFF9C4", "#F9A825"),
        ("⌀ Data Store",       "#FFCDD2", "#C62828"),
        ("⚠ Threat Node",      "#FF8A80", "#C62828"),
    ]
    lx = PAD_X + 80
    for i, (lbl, fc, sc) in enumerate(items):
        lines.append(
            f'<rect x="{lx+i*130}" y="{legend_y-12}" width="12" height="12" rx="2" fill="{fc}" stroke="{sc}" stroke-width="1.5"/>')
        lines.append(
            f'<text x="{lx+i*130+16}" y="{legend_y-2}" font-size="9" fill="#444">{lbl}</text>')
...
def show_architecture_diagram(workshop_config, threats=None, mode="architecture", key_suffix=""):
    """
    Render the draw.io-style architecture SVG inside a Streamlit expander.
    modes: architecture | threat | stride
    """
    mode_labels = {
        "architecture": "🏗️ Architecture Overview",
        "threat":       "🔴 Threat-Highlighted Architecture",
        "stride":       "🔵 STRIDE-Annotated Architecture",
    }
    label = mode_labels.get(mode, "Architecture Diagram")

    svg = render_architecture_svg(workshop_config, highlighted_threats=threats or [], mode=mode)

    # Scale to container width
    st.markdown(
        f'''<div style="overflow-x:auto;border:1px solid #e0e0e0;border-radius:8px;padding:8px;background:white">
{svg}
</div>''',
...
step_labels = ["Design", "Zones", "STRIDE Rules", "Attack Tree", "Identify", "Assess", "Done"]
step_values = [1, 2, 2.5, 3, 4, 5, 6]
step_icons  = ["🏗️","🏷️","⚡","🌳","🎯","📊","🏆"]
cols_steps  = st.columns(len(step_labels))
for col, icon, label, val in zip(cols_steps, step_icons, step_labels, step_values):
    with col:
...
step_num_map = {1:1, 2:2, 2.5:3, 3:4, 4:5, 5:6, 6:7}
current_num  = step_num_map.get(st.session_state.current_step, 1)
st.progress(current_num / 7)
st.markdown("---")
    ...
    with st.form("threat_selection_form"):
        ...
        with col1:
            ...
            user_component = st.selectbox(
                ...
                all_options,
                index=default_idx,
                help="Match to the affected component shown in the threat description above"
            )

            # STRIDE selector – sorted so correct answer isn't always in same position
            # but all 6 are shown (no invalid options)
            stride_default_idx = all_stride.index(threat_stride_cat) if threat_stride_cat in all_stride else 0
            user_stride = st.selectbox(
                "STRIDE Category (apply zone-direction rules above!):",
                all_stride,
                index=0,  # always start unselected so user has to think
                help="Use the zone-direction rule to determine which STRIDE category fits"
            )

            # Correct likelihood & impact from the predefined threat (validated)
            correct_likelihood = selected_predefined.get("likelihood", "Medium")
            correct_impact     = selected_predefined.get("impact", "Medium")

            user_likelihood = st.select_slider(
                "Likelihood",
                options=["Low", "Medium", "High", "Critical"],
                value=correct_likelihood,
                help=f"The validated likelihood for this threat is: {correct_likelihood}"
            )
            user_impact = st.select_slider(
                "Impact",
                options=["Low", "Medium", "High", "Critical"],
                value=correct_impact,
                help=f"The validated impact for this threat is: {correct_impact}"
            )
